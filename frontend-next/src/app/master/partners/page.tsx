...
type OrderMethod = "이메일" | "카카오톡" | "문자" | "엑셀 다운로드"
...
const initialSuppliers: Supplier[] = mockSuppliers.map((s, i) => ({
  ...s,
...
export default function Page() {
  ...
  const handleAdd = () => {
    if (!form.name) return
    const code = `S-${String(suppliers.length + 1).padStart(3, "0")}`
    setSuppliers(p => [...p, { code, ...form }])
  ...
  const handleSave = () => {
    if (!selected) return
    setSuppliers(p => p.map(s => s.code === selected.code ? selected : s))
    setSelected(null)
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="gap-1.5" onClick={handleSync} disabled={syncing}>
            ...
            {syncing ? "연동 중…" : synced ? "연동 완료" : "Z-MIS 연동"}
          </Button>
